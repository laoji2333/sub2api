    ...
    <div v-if="!isSimple" class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-emerald-600 dark:text-emerald-400">{{ moneyDisplaySymbol }}{{ formatBalance(balance) }}</p>
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-gray-900 dark:text-white">
            <span class="text-purple-600 dark:text-purple-400" :title="t('dashboard.actual')">{{ moneyDisplaySymbol }}{{ formatCost(stats?.today_actual_cost || 0) }}</span>
            <span class="text-sm font-normal text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')"> / {{ moneyDisplaySymbol }}{{ formatCost(stats?.today_cost || 0) }}</span>
          </p>
          <p class="text-xs">
            <span class="text-gray-500 dark:text-gray-400">{{ t('common.total') }}: </span>
            <span class="text-purple-600 dark:text-purple-400" :title="t('dashboard.actual')">{{ moneyDisplaySymbol }}{{ formatCost(stats?.total_actual_cost || 0) }}</span>
            <span class="text-gray-400 dark:text-gray-500" :title="t('dashboard.standard')"> / {{ moneyDisplaySymbol }}{{ formatCost(stats?.total_cost || 0) }}</span>
          </p>
        </div>
      </div>
    </div>
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <span class="font-mono text-sm text-purple-600 dark:text-purple-400" :title="t('dashboard.actual')">
            {{ moneyDisplaySymbol }}{{ formatCost(item.total_actual_cost) }}
          </span>
        </div>
        <div class="mt-2 space-y-1 text-xs">
          <div class="flex items-center justify-between">
            <span class="text-gray-500 dark:text-gray-400">{{ t('dashboard.todayCost') }}</span>
            <span class="font-mono text-gray-900 dark:text-white">{{ moneyDisplaySymbol }}{{ formatCost(item.today_actual_cost) }}</span>
          </div>
          <div class="flex items-center justify-between">
              ...
              <template v-else>
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span class="font-mono text-gray-700 dark:text-gray-200">
                    {{ moneyDisplaySymbol }}{{ formatUsd((quotaVal(item.quota, `${w}_usage_usd`) as number) ?? 0) }} / {{ moneyDisplaySymbol }}{{ formatUsd(quotaVal(item.quota, `${w}_limit_usd`) as number) }}
                  </span>
                </div>
                <div class="h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-700">
                  <div
