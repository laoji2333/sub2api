<template>
  <div v-if="usage" class="text-sm">
    <div class="flex items-center gap-1.5">
      ...
      <span class="font-medium text-gray-900 dark:text-white">{{ moneyDisplaySymbol }}{{ usage.today_actual_cost.toFixed(4) }}</span>
    </div>
    <div class="mt-0.5 flex items-center gap-1.5">
      <span class="text-gray-500 dark:text-gray-400">{{ t('admin.users.total') }}:</span>
      <span class="font-medium text-gray-900 dark:text-white">{{ moneyDisplaySymbol }}{{ usage.total_actual_cost.toFixed(4) }}</span>
    </div>
  </div>
