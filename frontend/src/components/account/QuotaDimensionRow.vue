  ...
  <div>
    ...
    <div class="flex items-center gap-2">
      <div :class="['relative', quotaNotifyGlobalEnabled ? 'flex-1 min-w-0' : 'flex-1']">
        <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 text-sm">{{ moneyDisplaySymbol }}</span>
        <input :value="limit" @input="onLimitInput" type="number" min="0" step="0.01" class="input py-1.5 text-sm" :style="moneyInputPaddingStyle" :placeholder="t('admin.accounts.quotaLimitPlaceholder')" />
      </div>
      <QuotaNotifyToggle
