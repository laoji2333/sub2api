      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button @click="$emit('clear-sticky-sessions', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="link" size="sm" class="text-amber-500" />
              {{ t('admin.accounts.viewStickySessions') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'clear-sticky-sessions', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow'])
const { t } = useI18n()
