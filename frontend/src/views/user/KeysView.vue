<template>
  <AppLayout>
            ...
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                ...
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ moneyDisplaySymbol }}{{ (usageStats[row.id]?.today_actual_cost ?? 0).toFixed(4) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-gray-500 dark:text-gray-400">{{ t('keys.total') }}:</span>
                <span class="font-medium text-gray-900 dark:text-white">
                  {{ moneyDisplaySymbol }}{{ (usageStats[row.id]?.total_actual_cost ?? 0).toFixed(4) }}
                </span>
              </div>
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  ...
                  <span :class="[
                    ...
                    {{ moneyDisplaySymbol }}{{ row.quota_used?.toFixed(2) || '0.00' }} / {{ moneyDisplaySymbol }}{{ row.quota?.toFixed(2) }}
                  </span>
                </div>
              ...
              <div v-if="row.rate_limit_5h > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ moneyDisplaySymbol }}{{ row.usage_5h?.toFixed(2) || '0.00' }}/{{ moneyDisplaySymbol }}{{ row.rate_limit_5h?.toFixed(2) }}
                  </span>
                </div>
              ...
              <div v-if="row.rate_limit_1d > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ moneyDisplaySymbol }}{{ row.usage_1d?.toFixed(2) || '0.00' }}/{{ moneyDisplaySymbol }}{{ row.rate_limit_1d?.toFixed(2) }}
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
              ...
              <div v-if="row.rate_limit_7d > 0">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span :class="[
                    ...
                    {{ moneyDisplaySymbol }}{{ row.usage_7d?.toFixed(2) || '0.00' }}/{{ moneyDisplaySymbol }}{{ row.rate_limit_7d?.toFixed(2) }}
                  </span>
                </div>
                <div class="h-1 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                  <div
      ...
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div class="space-y-3">
          ...
          <div class="space-y-4">
            <div>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{{ moneyDisplaySymbol }}</span>
                <input
            ...
            <div v-if="showEditModal && selectedKey && selectedKey.quota > 0">
              ...
              <div class="flex items-center gap-2">
                <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700">
                  <span class="font-medium text-gray-900 dark:text-white">
                    {{ moneyDisplaySymbol }}{{ selectedKey.quota_used?.toFixed(4) || '0.0000' }}
                  </span>
                  <span class="mx-2 text-gray-400">/</span>
                  <span class="text-gray-500 dark:text-gray-400">
                    {{ moneyDisplaySymbol }}{{ selectedKey.quota?.toFixed(2) || '0.00' }}
                  </span>
                </div>
                <button
                  type="button"
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ moneyDisplaySymbol }}{{ selectedKey.usage_5h?.toFixed(4) || '0.0000' }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ moneyDisplaySymbol }}{{ selectedKey.rate_limit_5h?.toFixed(2) || '0.00' }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ moneyDisplaySymbol }}{{ selectedKey.usage_1d?.toFixed(4) || '0.0000' }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ moneyDisplaySymbol }}{{ selectedKey.rate_limit_1d?.toFixed(2) || '0.00' }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-gray-100 px-3 py-2 dark:bg-dark-700 text-sm">
                    <span :class="[
                      ...
                      {{ moneyDisplaySymbol }}{{ selectedKey.usage_7d?.toFixed(4) || '0.0000' }}
                    </span>
                    <span class="mx-2 text-gray-400">/</span>
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ moneyDisplaySymbol }}{{ selectedKey.rate_limit_7d?.toFixed(2) || '0.00' }}
                    </span>
                  </div>
                </div>
                <div class="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
    ...
    <ConfirmDialog
      ...
      :message="t('keys.resetQuotaConfirmMessage', { name: selectedKey?.name, used: prefixMoney((selectedKey?.quota_used ?? 0).toFixed(4)) })"
...
const { moneyDisplaySymbol, moneyInputPaddingStyle, prefixMoney } = useMoneyDisplay()
