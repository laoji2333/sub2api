        ...
        <div v-if="order" class="rounded-xl bg-white p-5 shadow-sm dark:bg-dark-800">
          <div class="space-y-3 text-sm">
            ...
            <div v-if="hasAmountFields(order) && order.amount !== order.pay_amount" class="flex justify-between">
              ...
              <span class="font-medium text-gray-900 dark:text-white">{{ order.order_type === 'balance' ? prefixMoney(order.amount.toFixed(2)) : formatGatewayAmount(order.amount) }}</span>
            </div>
...
const i18n = useI18n()
const { t } = i18n
const { prefixMoney } = useMoneyDisplay()
